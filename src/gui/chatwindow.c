...
static void qq_chatwindow_init(QQChatWindow *win)
{
    ...
    //create header
    GdkPixbuf *pb = NULL;
    pb= gdk_pixbuf_new_from_file_at_size(IMGDIR"avatar.gif", 45, 45, NULL);
    ...
    gtk_box_pack_start(GTK_BOX(vbox), priv -> name, FALSE, FALSE, 2); 
    gtk_box_pack_start(GTK_BOX(vbox), priv -> lnick, FALSE, FALSE, 2); 
    ...
    gtk_box_pack_start(GTK_BOX(priv -> body_vbox), scrolled_win, TRUE, TRUE, 0); 
